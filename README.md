...
Review/edit `mcp-server.env` (`MCP_SERVER_PORT`, `AGENT_IP`, `MCPSERVER_IP`,
`OLLAMA_URL`, `MCP_AUTH_TOKEN`), then start it:
```bash
...
Review/edit `agent.env` (`AGENT_IP`, `MCPSERVER_IP`, `MCP_SERVER_PORT`,
`OLLAMA_URL`, `MCP_AUTH_TOKEN`) and make sure `mcp-server` is already
